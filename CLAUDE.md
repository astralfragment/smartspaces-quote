...
- **Storefront cart** is localStorage only for guests; logged-in customers also sync via a `quote.cart` customer metafield through `/api/proxy/cart`. Merge on hydrate: the most recent `updatedAt` wins per variant, with max qty as tiebreaker.
