...
- [ ] **Ship and enable the cart-validation Shopify Function** — covers the `/cart/<variant>:<qty>?replace` permalink bypass described in [docs/review/STATUS.md §3 stop-ship #3](review/STATUS.md). Until this is enabled, a quote-only item added via that URL reaches checkout at its (empty) retail price. Implementation: scaffold with `shopify app generate extension --type=function --template=cart-checkout-validation --name=quote-cart-validation` (JavaScript), write a `run()` that returns a validation error for each cart line whose `merchandise.product` has the `quote-only` tag, deploy with `shopify app deploy --force`. Merchant enable path: `Settings → Checkout → Checkout rules → Add validation → Smartspaces Quote → Quote cart validation`. **Hard blocker:** if that section is not visible in the admin, the store is on a plan that doesn't include checkout validation — the bypass cannot be closed without a plan upgrade.
